...
struct Printer< ::scout_msgs::ScoutStatus_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::scout_msgs::ScoutStatus_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "driver_states: ";
    if (v.driver_states.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.driver_states.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::scout_msgs::ScoutDriverState_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.driver_states[i]);
    }
    if (v.driver_states.empty() || false)
      s << "]";
    if (true || !indent.empty())
